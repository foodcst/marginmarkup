...
$("body").on("click", "#addmore", function() {
    var tr = "<tr><td><input type='text' class='description' name='description[]' style='width:260px;' required/></td><td><input type='text' class='pack' name='pack[]' style='width:30px;text-align:center;' required/></td><td><input type='text' class='size' name='size[]' style='width:50px;text-align:center;' required/></td><td><select class='purchase' name='purchaseunit[]' required><option value='case'>Case </option><option value='each'>Each </option></select></td><td><select class='um' name='um[]' style='width:50px;' required><option value='ea'>EA </option><option value='oz-wt'>OZ-wt </option><option value='oz-fl'>OZ-fl </option><option value='lb'>LB (wt 160z per lb) </option><option value='galoon'>Gallon (fluid,128 fl oz per gallon) </option></select></td><td><input type='text' class='ruperpu' name='ruperpu[]' style='width:50px;text-align:center;' required/></td><td><input type='text' class='yield' name='yield[]' style='width:50px;text-align:center;' required /></td><td><input type='button' class='removebutton' id='removemore'  value='Delete' /></td></tr>";
	 $('#table').append(tr);
});
...
<!-- left contaner start -->
  ...
  <aside id="officecontact">
 	<div class="order_back">
 	 ...
 	 <a href="<?php echo base_url();?>orderguide/cat/1">Back to Order Guide </a> 
	 </div>
   ...
   <div id="profile">
		 ...
		 <!--Tab Start -->
				 ...
				 <div class="mini-tab-grocery " >
				 		     ...
				 		     <div id="tab_manage" class="tab_manage"><!--common  -->  
							   <div class="content mCustomScrollbar"> <!--scroll -->
								  ...
								  <table id="table" border="0" cellpadding="3" cellspacing="10">
									<tr class="common-tr"> <th > Description </th><th >Pack</th><th >Size</th> <th style="width:50px;" >Purchase Unit</th> <th >U/M</th> <th >#RU per PU</th> <th >Yield % </th> <td> </td> </tr>
									<?php if(isset($orderguide)) {
									      foreach($orderguide as $guide) { 
									 ?>
									 <tr><td> <input type="hidden"  value="<?php echo $guide->grocery_id; ?>" name="grocery_id[]"  />
									 <input type="text" class="description" value="<?php echo $guide->grocery_desc; ?>" name="description[]" style="width:260px;" required /></td>
									 <td><input type="text" class="pack" name="pack[]" value="<?php echo $guide->grocery_pack; ?>" style="width:30px;text-align:center;" required /></td>
									 <td><input type="text" class="size" name="size[]" value="<?php echo $guide->grocery_size; ?>" style="width:50px;text-align:center;" required /></td>
									 <td>
										<select class="purchase" name="purchaseunit[]" required>
										  <option value="case">Case </option>
										  <option value="each">Each </option>
										</select>
										 </td>
									 <td>
										  <select class="um" name="um[]" style="width:50px;" required>
									...
									<td><input type="text" class="ruperpu" name="ruperpu[]" value="<?php echo $guide->grocery_ruperpu; ?>" style="width:50px;text-align:center;" required /></td> 
									<td><input type="text" class="yield" name="yield[]" value="<?php echo $guide->grocery_yiled; ?>" style="width:50px;text-align:center;" required /></td> 
									
										
									</tr>
									 <?php } } ?>
									  
									 <?php if(count($orderguide) == 0) { echo "<tr><td colspan='9'> No Order Guide for this category !.</td></tr>";
									 }?>
									
									</table>
									<?php if(count($orderguide) != 0) { ?>
						...
						<input type="button" id-"add-submit"  class="submit"   value="Add&Sync"/>
						</form>	 
						 </div>
						<br clear="all"	>
